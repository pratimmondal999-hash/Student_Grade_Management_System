...
def home():
    return jsonify({"message": "Student Grade Management Backend Running Successfully!"})
...
def add_student():
    ...
    try:
        cursor.execute(
            "INSERT INTO students (roll, name, marks) VALUES (?, ?, ?)",
...
def show_students():
    ...
    for roll, name, marks_json in rows:
        ...
        subjects = len(marks_list) if marks_list else 1
